...
SELECT AVG(preco_aluguel) FROM filmes; #Média de 2,20
...
FROM atuacoes
LEFT JOIN filmes
ON atuacoes.id_filme = filmes.id_filme #coluna em comum nas tabelas de atuações e filmes
LEFT JOIN atores
ON atuacoes.id_ator = atores.id_ator; #coluna em comum nas tabelas de atuações e atores
...
SELECT AVG(nota) FROM alugueis; #média 7,93
...
HAVING avaliacao_media >= (SELECT AVG(nota) FROM alugueis); #assim ele sempre pegará a média atual para fazer a busca
...
SELECT * FROM resultados; #Teste da view