...
SELECT * FROM alugueis; #578 registros de aluguéis visto em output
SELECT * FROM atores; #145 registros de atores visto em output
SELECT * FROM atuacoes; #213 registros de atuações visto em output
SELECT * FROM clientes; #123 registros de clientes visto em output 
SELECT * FROM filmes; #71 registros de filmes visto em output 
...
SELECT DISTINCT genero FROM filmes; #todos os gêneros estão no banco
...
SELECT DISTINCT estado FROM clientes; #Empresa atua em 11 estados, podendo expandir para mais 16 estados
...
SELECT * FROM clientes
ORDER BY nome_cliente; #se for decrescente acrescentar ao fim DESC
...
-- 3. Na tabela de clientes, tente descobrir qual foi o cliente com a data de criação de conta mais antiga, ou seja, o primeiro cliente cadastrado na base.
 ...
 LIMIT 1; #Acrescentar limite para saber apenas o mais antigo

-- 4. A empresa gostaria de saber quais são os TOP 5 filmes com a maior duração. Você seria capaz de fazer essa análise?

SELECT titulo, duracao FROM filmes #se não se importar em ver mais dados pode utilizar *
...
SELECT
	MAX(duracao),
    MIN(duracao)
FROM filmes; #Filme com duração máxima tem 200 minutos e com duração mínima tem 91 minutos

-- 5. Qual é a média de satisfação dos clientes da empresa em relação aos filmes alugados?

SELECT
	AVG(nota)
FROM alugueis; #Média de 7,93